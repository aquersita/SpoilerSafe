            ...
            <div className="max-w-[1600px] mx-auto px-6">
                {items.length === 0 ? (
                    <div className="bg-white border-2 border-dashed border-gray-200 rounded-xl p-16 text-center">
                        ...
                        <h3 className="text-xl font-black text-gray-900 mb-2">Tu watchlist está vacía</h3>
                        <p className="text-gray-500 mb-6">Explora el catálogo y guarda los animes que quieras ver.</p>
                    ...
                    <div className="space-y-4">
                        {items.map(anime => {
                            ...
                            const statusColor = anime.status === 'RELEASING' ? 'text-green-600 bg-green-50' : anime.status === 'NOT_YET_RELEASED' ? 'text-blue-600 bg-blue-50' : 'text-gray-600 bg-gray-100';

                            return (
                                <div key={anime.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow flex group">
                                    {/* Cover */}
                                    ...
                                    <div className="flex-1 p-4 md:p-5 flex flex-col justify-between min-w-0">
                                        <div>
                                            <div className="flex items-start justify-between gap-3">
                                                <h3 className="font-black text-gray-900 text-lg truncate cursor-pointer hover:text-primary transition-colors"
                                            ...
                                            <p className="text-xs text-gray-500 mt-1.5 truncate">{anime.genres?.join(', ')}</p>
                                        </div>
