...
const RSS_JSON_URL =
  'https://api.rss2json.com/v1/api.json?rss_url=https%3A%2F%2Fwww.animenewsnetwork.com%2Fnewsroom%2Frss.xml&count=20';

const CACHE_KEY = 'spoilersafe_news_v2';
...
function parseItems(items = []) {
  return items.map((item, i) => {
    const date = item.pubDate
      ...
      : '';
    const category = (item.categories?.[0] || 'Anime').replace(/^\w/, c => c.toUpperCase());
    const summary = item.description
      ? item.description.replace(/<[^>]*>?/gm, '').slice(0, 180).trim() + '…'
      : '';
    return {
      id: i,
      title: item.title || '',
...
const News = () => {
  ...
  useEffect(() => {
    ...
    const fetchNews = async () => {
      try {
        const res = await fetch(RSS_JSON_URL);
        const json = await res.json();
        if (json.status === 'ok' && json.items?.length > 0) {
          const parsed = parseItems(json.items);
          setNewsData(parsed);
